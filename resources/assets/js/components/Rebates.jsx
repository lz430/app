...
class Rebates extends React.PureComponent {
  ...
  componentWillReceiveProps(nextProps) {
    if (
      this.props.selectedRebates.length !== nextProps.selectedRebates.length
    ) {
  ...
  renderRebate(rebate, index) {
    ...
    const isSelectable = R.contains(rebate.id, this.state.compatibleRebateIds);
    const checkboxClass = `rebates__checkbox rebates__checkbox--inverted ${isSelected ? "rebates__checkbox--selected" : ""}`;

    return (
      <div
        onClick={isSelectable ? () => this.toggleRebate(rebate) : R.identity}
