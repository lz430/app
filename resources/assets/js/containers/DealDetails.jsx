...
class DealDetails extends React.PureComponent {
  ...
  allImages() {
    return R.concat(
      ...
      R.concat(this.state.fuelExternalImages, this.state.fuelInternalImages)
    );
  }

  renderDealRebatesModal() {
    return (
      <Modal onClose={this.props.clearSelectedDeal} closeText="Back to results">
  ...
  renderDot(photo, index) {
    const color = this.state.featuredImage.url === photo.url ? "gray" : "none";
    return (
      <svg
        key={index}
        style={{
    ...
    return (
      <Deal deal={deal} key={index} hideImageAndTitle={true}>
        <div className="deal-details__deal-content">
          ...
          <ul>
            {deal.versions[0].equipment.slice(0, 5).map((equipment, index) => {
              return <li key={index}>{equipment.name}</li>;
            })}
          </ul>
          ...
          <div className="deal-details__buttons">
            <button
              onClick={() => this.props.toggleCompare(this.props.deal)}
              className={
                "deal-details__button deal-details__button--small " +
                  (inCompareList ? "deal-details__button--blue" : "")
              }
            >
