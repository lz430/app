...
const rebates = {
    getAvailableTargetsForDeal: (dealTargets, deal) => {
     ...
     */
    getSelectedTargetsForDeal: (dealTargets, selectedTargets, deal) => {
        if (!(deal && dealTargets && dealTargets.hasOwnProperty(deal.id))) {
            return [];
        }

        let possibleTargetsForDeal = dealTargets[deal.id];

        return R.filter(possibleTargetForDeal => {
            return R.map(R.prop('targetId'), selectedTargets).includes(
